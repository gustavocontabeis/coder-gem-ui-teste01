...
@Component({
  selector: 'app-atributo-add',
  templateUrl: './atributo-add.component.html',
  styleUrls: ['./atributo-add.component.css']
})
export class AtributoAddComponent implements OnInit {
  ...
  salvar() {
    ...
    this.atributoService.adicionar(this.atributo).subscribe(resposta => {
      ...
      this.router.navigate(['/atributo/entidade-list']);
    }, error => {
      console.log(error);
      alert(error.ok);
    }
